...
if __name__ == "__main__":
    ...
    if run_case == "random_number":
        ...
        current_state = {"one": jnp.array([1., 0.]),"two": jnp.array([0., 1.])}
        ...
        for _ in tqdm(range(int(NOF_ITERATIONS/2))):
            current_state, current_key = run_samplers(sampler_list=[sampler_one, sampler_two],
                                                    state=current_state,
                                                    key=current_key)
        ...
        A_input = jnp.array([[2., 1.], [1., 2.]])
        b_input = jnp.array([3., 4.])
